...
export const createPlannedBudgetItem = async (req: AuthenticatedRequest, res: Response<ApiResponse>) => {
  try {
    ...
    if (!budget.grupoId.equals(groupId)) {
      return res.status(400).json({ success: false, error: 'O orçamento não pertence ao grupo especificado.' });
    }

    // Verify if the user is a member of the group associated with this budget
    const group = await Group.findById(groupId);
    if (!group || !group.membros.includes(userId)) {
      return res.status(403).json({ success: false, error: 'Você não tem permissão para adicionar itens a este orçamento.' });
    }
    ...
    res.status(500).json({ success: false, error: 'Server error creating planned budget item.' });
  }
};

// Get all planned budget items for a specific budget
export const getPlannedBudgetItemsForBudget = async (req: AuthenticatedRequest, res: Response<ApiResponse>) => {
  try {
    ...
    if (!group || !group.membros.includes(userId)) {
      return res.status(403).json({ success: false, error: 'Você não tem permissão para acessar os itens deste orçamento.' });
    }

    const plannedItems = await PlannedBudgetItem.find({ budgetId }).sort({ categoryType: 1, nome: 1 }).lean();
    ...
    res.status(500).json({ success: false, error: 'Server error fetching planned budget items.' });
  }
};

// Update a planned budget item
export const updatePlannedBudgetItem = async (req: AuthenticatedRequest, res: Response<ApiResponse>) => {
  try {
    ...
    if (!item) {
      return res.status(404).json({ success: false, error: 'Item de orçamento planejado não encontrado.' });
    }
    ...
    if (!budget) {
      return res.status(404).json({ success: false, error: 'Orçamento associado não encontrado.' });
    }
    const group = await Group.findById(budget.grupoId);
    if (!group || !group.membros.includes(userId)) {
      return res.status(403).json({ success: false, error: 'Você não tem permissão para atualizar este item.' });
    }
    ...
    res.status(500).json({ success: false, error: 'Server error updating planned budget item.' });
  }
};

// Delete a planned budget item
export const deletePlannedBudgetItem = async (req: AuthenticatedRequest, res: Response<ApiResponse>) => {
  try {
    const { itemId } = req.params;
    const userId = req.user?._id;
    ...
    const group = await Group.findById(budget.grupoId);
    if (!group || !group.membros.includes(userId)) {
      return res.status(403).json({ success: false, error: 'Você não tem permissão para deletar este item.' });
    }
    ...
    res.status(500).json({ success: false, error: 'Server error deleting planned budget item.' });
  }
};
