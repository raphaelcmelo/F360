...
export const createBudget = async (req: AuthenticatedRequest, res: Response<ApiResponse>) => {
  try {
    ...
    if (existingBudget) {
      return res.status(409).json({ success: false, error: 'Um orçamento para este grupo e período já existe.' });
    }
    ...
    res.status(500).json({ success: false, error: 'Server error creating budget.' });
  }
};

// Get a specific budget by ID
export const getBudgetById = async (req: AuthenticatedRequest, res: Response<ApiResponse>) => {
  try {
    ...
    if (!group || !group.membros.includes(userId)) {
      return res.status(403).json({ success: false, error: 'Você não tem permissão para acessar este orçamento.' });
    }
    ...
    res.status(500).json({ success: false, error: 'Server error fetching budget.' });
  }
};

// Get all budgets for a specific group
export const getGroupBudgets = async (req: AuthenticatedRequest, res: Response<ApiResponse>) => {
  try {
    ...
    if (!group || !group.membros.includes(userId)) {
      return res.status(403).json({ success: false, error: 'Você não tem permissão para acessar os orçamentos deste grupo.' });
    }

    const budgets = await Budget.find({ grupoId: groupId }).sort({ dataInicio: -1 }).lean();
    ...
    res.status(500).json({ success: false, error: 'Server error fetching group budgets.' });
  }
};

// Delete a budget
export const deleteBudget = async (req: AuthenticatedRequest, res: Response<ApiResponse>) => {
  try {
    const { budgetId } = req.params;
    const userId = req.user?._id;
    ...
    if (!budget.criadoPor.equals(userId)) {
      return res.status(403).json({ success: false, error: 'Você não tem permissão para deletar este orçamento.' });
    }
    ...
    res.status(500).json({ success: false, error: 'Server error deleting budget.' });
  }
};
